...
using GestionPapeleria.Vistas.Dashboard;
...
namespace GestionPapeleria
{
    public partial class Dashboard : Form
    {
        ...
        private GestorComboBox gestorComboBox;
        ...
        public Dashboard(String rol)
        {
            ...
            gestorComboBox = new GestorComboBox(cb_almacen, cb_proveedor, cb_marca);
            gestorComboBox.CargarComboBoxAlmacenes();
            gestorComboBox.CargarComboBoxProveedores();
            gestorComboBox.CargarComboBoxMarcas();
            gestorUsuarios = new GestorUsuarios(dataGridView_clientes, dataGridView_rolesUsuarios);
            gestorUsuarios.CargarClientes();
        ...
        private void filtroMarca()
        {
            ...
            try
            {
                SqlConnection con = new SqlConnection(GestionPapeleria.Auxiliar.GlobalVariables.DB_CONNECTION);
                con.Open();
                SqlDataAdapter adapter = new SqlDataAdapter(query, GestionPapeleria.Auxiliar.GlobalVariables.DB_CONNECTION);
                DataTable dt = new DataTable();
                adapter.Fill(dt);
        ...
        private void btn_alta_cliente_Click(object sender, EventArgs e)
        {
            ...
            if (btn_alta_cliente.Text.Equals("INSERTAR"))
            {
               ...
               bool exito = gestorUsuarios.InsertarCliente(tb_nombre_cli.Text, tb_correo_cli.Text, tb_telefono_cli.Text, tb_direccion_cli.Text, tb_username_cli.Text, tb_contrasena_cli.Text);
                if (exito)
                {
                    MessageBox.Show("Éxito");
                    limpiarDatos(); // Limpiar los datos de entrada
                    cargarClientes();
                }
            }
            else
